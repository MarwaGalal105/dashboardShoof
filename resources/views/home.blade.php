	                    ...
	                    <form id="lost_form" name="lost_form" class="lost_form_one" action="{{ Route('lostchild.store') }}" method="POST"  enctype="multipart/form-data" data-parsley-validate class="form-horizontal form-label-left" data-parsley-validate="" novalidate="">
							...
							<div class="form-group">
								<div class="col-md-12 col-sm-6 col-xs-12">
								  <input type="text" id="name" name="name" value="{{ old('name') }}"  placeholder="اسم المفقود" value="{{ old('name') }}" class="form-control form_control required datepicker hasDatepicker"  required="required" data-error="Name is required.">
						...
						<div class="item">
							<div class="ulockd-fservice-box text-right hvr-float-shadow">
								...
								<div class="db-details">
									<h4>{{ $info->name }}</h4>
									<p>{{ $info->notes }}</p>
									<a class="btn btn-lg ulockd-btn-styledark hvr-bounce-to-right" href="{{ url('findchild/'.$info->id) }}"> قراءه المزيد </a>
								</div>
							</div>
						</div>
						@endif
